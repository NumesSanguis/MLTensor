...
class DataInput():
    ...
    def read_from_txt(self):
        ...
        for i in range(5): #i = foldnumber
            header = True
            with open("data/fold_{}_data.txt".format(i)) as tsv:
                for line in csv.reader(tsv, dialect="excel-tab"): #You can also use delimiter="\t" rather than giving a dialect.
                    if header == True:
                        ...
                        if line[4] and line[4] != 'u': #needed to remove images without label
                            if line[4] == 'm': # 0 men
                                img_string_que[i].append(join('data','faces',line[0],'coarse_tilt_aligned_face.{}.{},{}'.format(line[2],line[1],0)))
                                img_label[i].append(0)
                            elif line[4] == 'f':  # 1 women
                                img_string_que[i].append(join('data','faces',line[0],'coarse_tilt_aligned_face.{}.{},{}'.format(line[2],line[1],1)))
                            ...
                            img_path[i].append(join('data','faces',line[0],'coarse_tilt_aligned_face.{}.{}'.format(line[2],line[1])))
