...
for i in range(5):
    in_path = join("data", "aligned", data[0][i][0], "landmark_aligned_face.{}.{}".format(data[0][i][2], data[0][i][1]))
    print(in_path)
...
#print(input_images[0][0].eval().shape)
print(input_images[0].get_shape().as_list())  # TODO Why [None, None, 3]???

#816x816 pixels
#vector_size = input_images[0].height * input_images[0].width * input_images[0].channels
vector_size = 816 * 816 * input_images[0].get_shape().as_list()[2]
