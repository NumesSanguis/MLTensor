...
def inference(images):
    ...
    with tf.variable_scope('conv1') as scope:
        kernel = tf.Variable(tf.random_normal([5, 5, 3, 64]), name="conv1_weights")
        conv = tf.nn.conv2d(images, kernel, [1, 1, 1, 1], padding='SAME')

        biases = tf.Variable(tf.zeros([64]), name="conv1_biases")
    ...
    with tf.variable_scope('conv2') as scope:
        kernel = tf.Variable(tf.random_normal([5, 5, 64, 64]), name="conv2_weights")
        conv = tf.nn.conv2d(norm1, kernel, [1, 1, 1, 1], padding='SAME')
        biases = tf.Variable(tf.zeros([64]), name="conv2_biases")
        bias = tf.nn.bias_add(conv, biases)
